...
namespace rude 
{
	namespace config { class ConfigImpl; }
	...
	public:
		...
		// Note: due to MSFS WASM restrictions, files can only be written to "\work" folder.
		// That folder is added to the passed filename, so don't pass "//work//" to the library, only the filename.
		//=
		static void setDefaultConfigFile(const char* filename);
		...
		//=
		// Set's the default filename to be used by the load() and save() methods
		// The default filename is initially set to "default.ini"
		...
		//=
		// Returns the default filename to be used by the load() and save() methods
		// The default filename is initially set to "default.ini"
		// Affects the current instance only
		//=
		...
		// </code>
		//=
		bool getBoolValue(const char* name, bool defaultValue = false) const;
		...
		// Returns the passed default value if the data does not exist or if an integer value cannot be determined.
		//=
		int getIntValue(const char* name, int defaultValue = 0) const;

		//=
		// Returns the double value, or the passed default value if a double value cannot be determined
		// Thie is a helper function that converts the string value stored in the config object to a double
		//=
		double getDoubleValue(const char* name, double defaultValue = 0) const;
		...
		//=
		const char* getValue(const char* name, const char* defaultValue = 0) const;
