...
#include "../Headers/RealOrganiser.h"
#include "../Headers/ParserJuly2004.h"
#include "../Headers/Base64Encoder.h"
...
namespace rude 
{
	namespace config 
	{
		// STATIC DATA
		//
		std::string ConfigImpl::s_defaultConfigFile = "\\work\\default.ini";
		...
		void ConfigImpl::setDefaultConfigFile(const char* filename)
		{
			if (filename)
			{
				s_defaultConfigFile = "\\work\\";
				s_defaultConfigFile += filename;
			}
			else s_defaultConfigFile = "\\work\\default.ini";
		}
		...
		//
		bool ConfigImpl::stringToBool(const char* value, bool defaultValue)
		{
			if (value)
			{
				...
				case 'o':
					...
					if (value[1] == 'n' || value[1] == 'N') return true;
					else return false;
		...
		int ConfigImpl::stringToInt(const char* string, int defaultValue)
		{
			int result = defaultValue;

			if (string && sscanf(string, "%d", &result) == EOF) return defaultValue;
		...
		double ConfigImpl::stringToDouble(const char* string, double defaultValue)
		{
			double result = defaultValue;

			if (string && sscanf(string, "%lf", &result) == EOF) return defaultValue;
		...
		void ConfigImpl::setConfigFile(const char* filename)
		{
			if (filename)
			{
				d_configfile = "\\work\\";
			...
			else d_configfile = s_defaultConfigFile.c_str();
		}
		...
		const char* ConfigImpl::getStringValue(const char* name, const char* defaultValue) const
		{
			const char* result = d_file->getStringValue(name);
