...
#include "../Headers/AbstractWriter.h"
...
namespace rude 
{
	namespace config 
	{
		...
		File::~File()
		{
			...
			for (iter = d_sections.begin(); iter != d_sections.end(); iter++) delete (*iter);
		}
		...
		void File::acceptWriter(AbstractWriter& writer) const
		{
			...
			for (iter = d_sections.begin(); iter != d_sections.end(); iter++) (*iter)->acceptWriter(writer);
		}
		...
		{
			if (d_currentSection) return d_currentSection->getValue(name);
		...
		{
			if (d_currentSection) return d_currentSection->deleteData(name);
		...
		{
			if (d_currentSection) d_currentSection->setValue(name, value);
		}


		int File::getNumDataMembers() const
		{
			if (d_currentSection) return d_currentSection->getNumDataMembers();
		...
		{
			if (d_currentSection) return d_currentSection->getDataNameAt(index);
		...
		{
			if (d_currentSection) return d_currentSection->getDataValueAt(index);
		...
		{
			if (d_currentSection) return d_currentSection->exists(name);
		...
		const char* File::getSectionNameAt(int index) const
		{
			...
			for (int x = 0; x < len; x++)
			{
				...
				{
					if (position == index) return theSection->getSectionName();
		...
		bool File::deleteSection(const char* sectionname)
		{
			if (sectionname)
			{
				...
				if (thesection)
				{
					...
					if (thesection == d_currentSection) return setSection("", true);
