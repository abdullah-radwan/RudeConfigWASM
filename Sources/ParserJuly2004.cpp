...
#include "../Headers/AbstractOrganiser.h"
	...
	namespace config 
	{
		...
		bool ParserJuly2004::chompEOL(std::istream& inputstream)
		{
			...
			if (isEOL(c))
			{
				...
				if ((c != next_c) && isEOL(next_c)) inputstream.get();
			}
				...
				else if (c == '[')
				{
					...
					while (sectionState != ENDSECTION)
					{
						...
						case STARTSECTION:						
							...
							// discard whitespace and loop
							else if (c == ' ' || c == '\t') infile.get();
							else if (c == ']')
							{
								// discard ']'
								infile.get();
						...
						case ENDSECTIONID:
							...
							if (c == EOF || isEOL(c)) sectionState = FOUNDIDONLY;
						...
						case SECTIONCOMMENT:
							...
							// append to comment and loop
							else comment += infile.get();
				...
				else
				{
					...
					while (kvState != ENDKEYVALUE)
					{
						...
						case KEY:
						{
							...
							if (c == EOF || isEOL(c)) kvState = ENDKV;
						...
						case QUOTEVALUE:
						{
							...
							// append to value and loop
							else value += infile.get();
						...
						case NONQUOTEESCAPE:
						{
							...
							else
							{
								...
								if (isEOL(c) && isEOL(next_c) && (c != next_c)) value += infile.get();
