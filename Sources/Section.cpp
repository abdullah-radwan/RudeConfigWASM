...
namespace rude 
{
	namespace config 
	{
		...
		Section::~Section()
		{
			...
			for (iter = d_allDataVector.begin(); iter != d_allDataVector.end(); iter++) delete (*iter);
		}
		...
		void Section::acceptWriter(AbstractWriter& writer) const
		{
			...
			for (iter = d_allDataVector.begin(); iter != d_allDataVector.end(); iter++) (*iter)->acceptWriter(writer);
		}
		...
		void Section::isDeleted(bool is_it_or_not)
		{
			...
			if (d_isDeleted)
			{
				...
				for (iter = d_allDataVector.begin(); iter != d_allDataVector.end(); iter++) (*iter)->isDeleted(true);
		...
		const char* Section::getValue(const char* name) const
		{
			if (name)
			{
				...
				if (mydata) return mydata->getValue();
			}
		...
		const char* Section::getComment(const char* name) const
		{
			if (name)
			{
				...
				if (mydata) return mydata->getComment();
			}
			return "";
		}
		...
		void Section::setComment(const char* name, const char* comment)
		{
			if (name)
			{
				...
				if (mydata) mydata->setComment(comment);
			}
		}
